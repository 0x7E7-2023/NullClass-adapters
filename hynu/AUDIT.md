...
| # | 读什么 | 代码位置 | 取出的值 | 去处 |
|---|---|---|---|---|
| 1 | 课表表格：`#timetable` / `#kbtable`，都没有时含 `.kbcontent` 的那张表 | `findTable()` → `rowsOfTable()` | 每个单元格的文字 + `div.kbcontent` 的原始 HTML + 该格的 `colspan` / `rowspan` 两个**属性值**（见下） | 载荷 `rows[]`（由 `parse.js` 解释） |
...
第 1 项的 `colspan` / `rowspan` 是 **2026-09-12 才读的**：`cellsOfRow()` 用它们按行还原表格的网格
（`colspan` 让本行后面的格子右移，`rowspan` 让后面几行的这一列空出来），给每个格子算出 `col`
（网格列号）与 `span`（跨列数）交给 `parse.js`。读的只是这两个**布局属性值**（`parseInt` 成数字），
不是文字内容；页面上其它属性、其它元素一律不读，载荷里也只多这两个数字字段。
...
## 取列：星期按网格列号对齐（2026-09-12 修过一次）

上游把「第几个格子」直接当星期几，课表首列是节次列时整表错一天。移植后改成按**网格列号**对齐，
2026-09-12 又修过一次无表头时的兜底（缺陷单：无表头 + 行中部有合并格）：

1. 表头认出来时，「星期 X」标签所在格的 `col` 就是那一天，格子按网格列号取（跨列的合并格在它
   覆盖到的每一列上都能取到）。
2. **表头认不出来时**，兜底窗口的起点按**表宽**算（`表宽 - 7`，即「最后 7 列是周一到周日」），
   **不按某一行的格子数**：行中部有合并格的那一行会少一个格子，按本行格子数算会把整行挪一天
   （星期一的课被读成星期二）。表宽由 `extract.js` 交出的 `col` + `span` 推出，与行的参差无关。
3. 表头只认出一部分天时，剩下的天按同一条表宽规则补上，但**不占已被别的天占用的列**；
   补不上的那一列不猜（宁可少认一天，也不把课排到错的星期）。
4. 拿不准的地方一律进 `warnings` 指名道姓：兜底推出来的列范围、**有几条安排的星期是推的**、
   跨列合并格（一个格子的课会落到它覆盖的每一天）、以及没能对上的天。不会静默错位。
...
- 学期下拉框的 id 里含 `xnxq` 是强智的通行写法（`xnxq01id`），同样没在真机验过；
  ...
  不猜学期名。
- fixture 是**合成的**（见 `fixtures/*.extracted.json` 的 `_note`）：它只保证同一份输入
  永远得到同一份输出，**不保证**真实页面上的解析是对的。`noheader.extracted.json` 专测无表头
  兜底（含行中部合并格、跨行节次格），`basic.extracted.json` 的表头是按 `col` 对齐的那条路径。
