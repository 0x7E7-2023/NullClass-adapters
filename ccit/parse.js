(function () {
    // 长春工程学院（强智 · 高校综合管理教务系统 /jsxsd/，走 WebVPN）课表解析
    // 移植自 shiguang_warehouse 的 CCIT/ccit.js（MIT，上游作者 星河欲转）
    //   https://github.com/XingHeYuZhuan/shiguang_warehouse
    //   上游快照 commit e62554a4034386b893bcd6813c7b2b64f8c730a3（2026-09-12）
    //
    // 上游的核心是 parseTimetableToModel() + parseWeeks() + applyCustomTimeSplitting()：
    // 读 #kbtable 每个 td 里 div.kbcontent 的明细，课程名取第一个非空文本节点，
    // font[title=老师|教师|教室|周次(节次)] 取教师/教室/周次，节次从周次串的方括号里抠
    //（[01-02节]），一个格子里多门课用一长串减号分隔。
    // 这里保持同一套字段来源，改动集中在七处（逐条见 AUDIT.md §6）：
    //   ① 星期按**网格列号**对齐（extract.js 每个格子都带 col/span，跨行/跨列的合并单元格
    //      也算得对）：课表首列是「节次」列，它常常 rowspan 跨两行，那一行的 td 会整体前移
    //      一格 —— 上游按「第几个 td 就是星期几」算会把整行错一天。表头行用来认
    //      「星期 d 在第几列」，认不出时按表宽推断并进 warnings；
    //   ② 周次按**逗号分段、各段自判单双**。上游 weekStr.split('(')[0] 会把括号**整段**丢掉，
    //      于是 1-16周(单)、1-16(单周) 都塌成「每周都上」，(单)1-16周 更是连一周都不剩；
    //      这里四种写法都认（单双写在「周」后 / 写在数字前 / 写在括号里），
    //      认不出单双的（「单双周」「隔周」）按每周处理并出声，不许静默改写；
    //   ③ 「周」字写在括号里（1-16(周)）是**正常形态**；括号里只有数字或数字区间的才是
    //      教学班序号（(1)、(1-2)），不当周次也不当周字；
    //   ④ 节次除连字符外也认逗号写法（[05,06节]，上游 split('-') 只取到一个数就整块丢弃）
    //      与连写（[0304节]），并且**在解析阶段就夹住上限**（MAX_PERIOD 节）：越界的块按
    //      「读不出节次」处理 —— 不夹的话补占位作息会补出 24:00 这种非法时刻，
    //      整次导入会被载荷校验拒收（不是跳过一节，是整包被拒）；
    //   ⑤ 上游的 applyCustomTimeSplitting() 按上课地点给「错峰」课程改上下课时间。
    //      我们的课表载荷只有一份学期级作息表，放不下每门课各自的时刻 ——
    //      按移植手册 §4.4 前半条：有节次就按节次放，自定义时间丢掉**但出声**
    //      （哪些课属于错峰作息，用户在「学期管理」里能自己核对）；
    ...
    var SCHOOL_NAME = '长春工程学院';

    // 学校作息时间（长春工程学院全年统一作息，12 节）。上游硬编码在
    ...
    var PERIOD_TIMES = [
        ...
        { periodIndex: 7, start: '15:35', end: '16:20' },
        { periodIndex: 8, start: '16:25', end: '17:10' },
        { periodIndex: 9, start: '18:00', end: '18:45' },
        { periodIndex: 10, start: '18:50', end: '19:35' },
        { periodIndex: 11, start: '19:40', end: '20:25' },
        { periodIndex: 12, start: '20:30', end: '21:15' }
    ];
    var PERIOD_COUNT = PERIOD_TIMES.length;

    // 节次上限：越过它一定是脏数据（合法作息不可能到 17 节以上）。
    // 这个数与下面「补占位」的公式是一对：占位时刻写成 pad2(7 + 节次)，第 16 节 =
    // 23:00-23:45 是最后一个还在当天的档位，再往后补出来的就不是 00:00-23:59 的时刻了，
    // 而载荷校验的时间正则只收 HH:mm —— 那样的载荷整次导入都会被拒收。
    var MAX_PERIOD = 16;

    // 上游写死的学期总周数（saveAppConfig 里 semesterTotalWeeks: 20，教务页面不给这个值）
    ...
    // 方括号里的是节次。带 /g 的那份用来**逐对**扫出「一段文本里并排的两组周次」
    //（1-8周[01-02节],10-16周[03-04节]），必须每段都读到，不能只取第一对。
    var WEEK_BRACKET_ALL = /[\[【]\s*([^\]】]*)\s*[\]】]/g;
    // 区间分隔符：教务页面上出现过全角波浪 ～(U+FF5E)、全角减号 －(U+FF0D)、数学减号
    // −(U+2212)、en dash –(U+2013)、em dash —(U+2014)、半角波浪 ~、汉字「至」（以及「到」）。
    // 取区间**之前**先统一成半角连字符 —— 漏认一个字符，区间就读不成区间，会掉进下面的
    // 「单数字回退」：整门课静默塌成只上第 1 周（1～16周 → w1-1，丢 15 周，还没有告警）。
    var RANGE_SEP = /[～－−–—~至到]/g;
    var WEEK_RANGE = /(\d{1,2})-(\d{1,2})/;
    ...
    var SEGMENT_SPLIT = /[\s,，、;；]+/;
    // 错峰作息的两组地点正则：上游 applyCustomTimeSplitting() 里逐字照搬。
    // 第一组是 07:50 起、第二组是 08:10 起（与内置的 08:00 起各差约 10 分钟）。
    var STAGGER_GROUPS = [
        /(DJ1-1[12])|(DJ2-2[1-3])|(XJ3-3[12])|(XJ5-5[12])|(XJ6-6[12])|(XJ7-7[1-4])/,
        /(DJ1-1[3-5])|(DJ2-2[4-6])|(XJ3-3[34])|(XJ6-63)|(XJ7-7[5-7])|(DS)|(XS)/
    ];
    ...
    // 提取时刻那一周的周一。移植手册 §4.3：firstDay 是第 1 周的第一天，上游
    // firstDayOfWeek=1（周一），所以推算值取「回退到周一」的那天。
    // 不用 Date.parse：Rhino 对 ISO 串的支持不齐。非法日期（如 2026-02-30）会被
    // Date 归一成 3 月 2 日 —— 那是教务自己给错，不在这里拦。
    ...
    function nameOf(blockHtml) {
        var lines = String(blockHtml).split(/<font/i)[0].split(/<br\s*\/?>/i);
    ...
    //
    // **一个块里不止一段周次**（两种形态都要逐段产出，命中第一段就 return 是错的）：
    //   ① 同一格写两个 font："1-8周[01-02节]" + "10-16周[03-04节]"
    //   ② 一段文本里并排两组："1-8周[01-02节],10-16周[03-04节]"
    // 两种情况的后一段都带自己的节次，必须各按各的段产出一条安排；只取第一段的话，
    // 第二段整段静默消失（周次、节次一起丢，连 warnings 里都没有一条）。
    function specOf(blockHtml) {
        ...
        // 只有 font[title=节次] 的写法：周次不在这条字段里
        var only = titledText(fonts, /^节次$/);
        return only ? [{ weeks: '', periods: only }] : [];
    }

    // 一段「周次(节次)」文本 → 若干 { weeks, periods }：按方括号**逐对**切。
    // 没有方括号时整段都是周次（节次留给 font[title=节次] 或按「没认出节次」报出来）。
    function pushSpecs(out, text) {
        var body = clean(text);
        var re = WEEK_BRACKET_ALL;
        re.lastIndex = 0;
        var m = re.exec(body);
        if (!m) {
            out.push({ weeks: body, periods: '' });
            return;
        }
        var at = 0;
        while (m) {
            out.push({ weeks: clean(body.substring(at, m.index)), periods: clean(m[1]) });
            at = m.index + m[0].length;
            m = re.exec(body);
        }
        // 末尾还挂着一截没有方括号的周次（"1-8周[01-02节]10-16周"）：也当作一段，
        // 节次留空 —— 由调用方按「没认出节次」跳过并点名，不许静默吞掉。
        var tail = clean(body.substring(at));
        if (tail) out.push({ weeks: tail, periods: '' });
    }
    ...
    //   1-16周 / 1-16周(单) / 1-15周(双)         单双标记写在「周」字后面
    //   1-16(周)                                 「周」字写在括号里 —— 强智的常见形态
    //                                            （cqrk 的注释就是 '1-9,11-17(周)[01-02节]'）
    //   (单)1-16周 / 双周2-16 / 1-16周 双          单双写在数字**前面**、或被空白隔开
    //   1-16(单周)                               单双与周字一起写在括号里
    //   (1)1-16周 / 9-10周(1-2)                  括号里的纯数字/区间是教学班序号，不是周次
    //   / 1-3,5周 / 第3周 / 1-8周(单),10-16周(双)
    //
    // 三条规矩（都与上游不同，上游是 weekStr.split('(')[0] 之后按逗号数数字）：
    //   · **不按逗号 split 之后再数数字**：先分段，每段自己判单双 —— 上游整串砍掉 '('
    //     之后，'1-16周(单)' 的单双标记整段消失（塌成每周都上）、'(单)1-16周' 更是
    //     连一周都不剩（整门课静默丢周次）。
    ...
    function weeksIn(source) {
        ...
        if (!text) return [];
        // 分隔符归一到半角连字符（见 RANGE_SEP 注释）：先归一，再取区间。
        // "1-16周" 这种标准形态归一后原样不变，结果不受影响。
        text = text.replace(RANGE_SEP, '-');
        ...
        // 区间两端的空白吃掉（"1 - 16 周"）：按空白分段时区间才不会被拆成两个孤立周次
        //（分隔符上面已经归一成半角连字符了，这里只收它两侧的空白）
        text = text.replace(/(\d)\s*-\s*(\d)/g, '$1-$2');
        var segments = text.split(SEGMENT_SPLIT);
        // 被逗号一类分隔符切成独立一段的单/双（"1-16周,双"）并给最近的周次段：并列标记不许丢。
        // 空白也是分段符（SEGMENT_SPLIT 里收着 \s），"1-16周 双" 这种标记自成一段的写法
        // 走下面的并段逻辑；标在数字**前面**的（"双周2-16"）本来就在同一段里，也不用并。
        ...
        for (var s = 0; s < segments.length; s++) {
            ...
            var range = WEEK_RANGE.exec(segment);
            if (range) {
    ...
    // 节次：方括号里的内容。"01-02节" / "1-2节" / "0304节" / "05,06节" / "第9,10节" 都认；
    // 上游用 split('-') 只取首尾两个数，逗号写法（[05,06节]）会读成 NaN 而整块被丢。
    ...
    function periodsIn(source) {
        ...
        // 分隔符与 weeksIn 同一口径先归一：[01～02节] 这类全角分隔符原本读不出来，
        // 整块会被当成「没认出节次」丢掉（有声，但不是我们想要的结果）
        body = body.replace(RANGE_SEP, '-');
        if (!body) return null;
        var start = null;
        var end = null;
        function note(value) {
    ...
    // 「2026-2027-1」→「2026-2027学年第一学期」（教务没给学期名时用它）
    ...
    // 连学期名和学期号都没有时的兜底：按**提取时刻**推学年学期（8 月及以后算本学年
    // 第一学期，2-7 月算上一学年第二学期，1 月算上一学年第一学期）。这是推算，会进 warnings。
    function termNameFromNow(isoDate) {
        var m = /^(\d{4})-(\d{1,2})-(\d{1,2})/.exec(clean(isoDate));
        if (!m) return '';
        var year = Number(m[1]);
        var month = Number(m[2]);
        var startYear = month >= 8 ? year : year - 1;
        var half = (month >= 2 && month <= 7) ? '第二学期' : '第一学期';
        return startYear + '-' + (startYear + 1) + '学年' + half;
    }
    ...
    // 没有 col 的输入（手写的用例）退回「第几个格子」—— 与历史行为一致。
    ...
    // 每一列上都能取到（这样的格子里如果有课，会落到它跨的每一天，进 warnings 说明）。
    ...
    // 行中部有合并格时那一行会少一个格子，按格子数算会整行错一天。
    var tableWidth = Number(data.cols) || 0;
    if (!(tableWidth > 0)) {
        for (var tr = 0; tr < rows.length; tr++) {
            for (var tc = 0; tc < rows[tr].length; tc++) {
                var at = colOf(rows[tr][tc], tc) + spanOf(rows[tr][tc]);
                if (at > tableWidth) tableWidth = at;
            }
        }
    }

    // 星期表头：找「没有课程内容、且有 ≥4 个星期标签」的那一行，按它的列号取每天那一列。
    // 两边都记着：headerLabelCells 是**带星期标签的格子数**（同一天出现两次也算两个），
    // headerLabels 是**认出来的不同天数**（最多 7）—— 前者报「表被拉宽了」，后者报「有几天没认出来」。
    var headerRow = -1;
    ...
    for (var hr = 0; hr < rows.length; hr++) {
        ...
        for (var hc = 0; hc < rows[hr].length; hc++) {
            if (rows[hr][hc].parts && rows[hr][hc].parts.length) rowHasContent = true;
        ...
        if (rowHasContent || labels < 4) continue;
        if (labels > headerLabels) {
            headerLabels = labels;
            headerLabelCells = cellLabels;
            headerRow = hr;
            dayCol = cols;
        }
    }

    // 认不出表头时的兜底窗口起点：整张表的**最后 7 列**是星期一到星期日。
    // 强智的表是「节次 + 星期一到星期日」8 列，最后 7 列正好是星期；
    // 表宽不到 8 列时窗口从第 1 列起，但首列是节次标签的话往后挪一格。
    ...
    var order = [];
    var byCourse = {};
    // 星期是按表宽推出来的课程格子数：这些安排的星期可能不准，warnings 里要说一声
    var guessedCells = 0;
    ...
    for (var ri = 0; ri < rows.length; ri++) {
        ...
        var row = rows[ri];
        // 跨列且有内容的格子：它覆盖到的每一天都会拿到这同一格 —— 先数一遍（每个格子只数一次）
        for (var mc = 0; mc < row.length; mc++) {
            if (spanOf(row[mc]) < 2) continue;
            if (!(row[mc].parts && row[mc].parts.length)) continue;
            var mcStart = colOf(row[mc], mc);
            var coveredDays = 0;
            for (var cd = 1; cd <= 7; cd++) {
                if (dayColOf[cd] >= mcStart && dayColOf[cd] < mcStart + spanOf(row[mc])) coveredDays++;
            }
            if (coveredDays > 1) mergedCells++;
        }
        // 逐个**网格列**取格子 —— 不按数组下标：强智的首列是节次列，被 rowspan 跨掉的
        // 那一行里第一个 td 落在 col 1，按下标算会把整行错一天。
        for (var day2 = 1; day2 <= 7; day2++) {
            var want = dayColOf[day2];
            if (want < 0) continue;
            var cell = cellAtGridCol(row, want);
            if (!cell) continue;
            var parts = cell.parts || [];
            if (!parts.length) continue;
            if (dayGuessed[day2]) guessedCells++;
            for (var p = 0; p < parts.length; p++) {
                ...
                for (var b = 0; b < blocks.length; b++) {
                    ...
                    // 一个块里可能写了**多段**周次（两个 font，或一段文本里并排两组）：
                    // 逐段产出安排，节次各按各的段走 —— 不能只认第一段。
                    var specs = specOf(blockHtml);
                    if (!specs.length) {
                        noPeriodBlocks++;
                        noteDropped(name, teacher, '没认出节次');
                        continue;
                    }
                    // 这一段块里有没有产出过安排：错峰地点的计数按块算，不按段算
                    var imported = false;
                    for (var sp = 0; sp < specs.length; sp++) {
                        ...
                        if (periods.end > MAX_PERIOD) {
                            ...
                            // 「读不出节次」处理。**不能**照单收下 —— 补占位作息会补出 24:00 这种
                            // 非法时刻，载荷校验一拒，整次导入就白导了。
                            overPeriodBlocks++;
                            noteDropped(name, teacher, '节次超出 ' + MAX_PERIOD + ' 节');
                            continue;
                        }
                        var weeks = specs[sp].weeks ? weeksIn(specs[sp].weeks) : [];
                        if (!weeks.length) {
                            skippedBlocks++;
                    ...
                    // 错峰作息：地点落在上游那两组正则里的课（它们在教务里是另一套上下课
                    // 时间）。载荷只有一份学期级作息，放不下每门课的时刻 —— 出声，不静默丢。
                    var loc = roomOf(fonts);
                    if (loc && isStaggered(loc) && staggerNames.indexOf(name) < 0) {
                        staggerCount++;
                        if (staggerNames.length < 3) staggerNames.push(name);
                    }
                }
            }
        }
    ...
    function isStaggered(location) {
        for (var i = 0; i < STAGGER_GROUPS.length; i++) {
            if (STAGGER_GROUPS[i].test(location)) return true;
        }
        return false;
    }
    ...
    var termNameInferred = false;
    if (!termName) {
        termName = SCHOOL_NAME + termNameFromNow(data.now);
        termNameInferred = true;
    }
    ...
    // 推算/假定出来的东西逐条说清楚（移植手册 §4.2）：这些值在库里和真值长得一模一样，
    ...
    if (skippedBlocks > 0 || noPeriodBlocks > 0 || overPeriodBlocks > 0) {
        var skipParts = [];
        if (skippedBlocks > 0) skipParts.push(skippedBlocks + ' 个没认出周次');
        if (noPeriodBlocks > 0) skipParts.push(noPeriodBlocks + ' 个没认出节次');
        if (overPeriodBlocks > 0) skipParts.push(overPeriodBlocks + ' 个节次超出 ' + MAX_PERIOD + ' 节');
        // 被跳过的块按名字点名（最多 5 个）—— 名单可能很长，pushWarning 会把它截到 200 字以内
        pushWarning('有 ' + skipParts.join('、') + ' 的课程块已跳过（教务页面结构可能已调整，欢迎反馈）。' +
            droppedText());
    }
    if (ambiguousMarks > 0) {
    ...
    if (staggerCount > 0) {
        pushWarning('有 ' + staggerCount + ' 门课的上课地点属于学校的错峰作息（不同教学楼/楼层的' +
            '上下课时间相差约 10 分钟），课表已统一按内置作息时间排列，时间对不上请到「学期管理」里核对。' +
            '涉及：' + staggerNames.join('、'));
    }
    if (headerLabels === 0) {
        pushWarning('课表里没找到星期表头：已按表宽 ' + tableWidth + ' 列推算（从第 ' +
            (fallbackStart + 1) + ' 列起依次为星期一到星期日），课表可能整体错位，请核对导入结果');
    } else {
        if (headerLabels < 7) {
            pushWarning('星期表头只认出了 ' + headerLabels + ' 天' +
                (guessedDays > 0 ? '，另外 ' + guessedDays + ' 天是按表宽推断的' : '') +
                '，请核对导入结果');
        }
    ...
    // 定不下列的那些天：课直接不导入 —— 无论表头认没认出来都要说，不许静默少课
    if (unplacedDays > 0) {
        pushWarning('有 ' + unplacedDays + ' 天在课表里定不下列（表头没认出、按表宽推也越界或撞车），' +
            '这些天的课没有导入 —— 请把教务页面的课表截图反馈给适配器作者');
    }
    if (guessedCells > 0 && headerLabels > 0) {
        pushWarning('有 ' + guessedCells + ' 个课程格子的星期是按表宽推断的（星期表头没认全），请核对导入结果');
    }
    if (mergedCells > 0) {
        pushWarning('课表里有 ' + mergedCells + ' 个格子跨了多天（合并单元格）：格里的课被放到了它跨的每一天，' +
            '请核对导入结果');
    }
    if (termNameInferred) {
        pushWarning('没能从教务页面读出学年学期，学期名已按提取日期推成「' + termName +
            '」，请在「学期管理」里核对并改名');
    }
