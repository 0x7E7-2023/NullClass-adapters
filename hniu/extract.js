(function () {
    // 湖南信息职业技术学院教务适配器 —— 强智科技「高校综合管理教务系统」（学生端 /jsxsd/）。
    // 移植自 shiguang_warehouse 的 HNIU/hniu_01.js
    //   https://github.com/XingHeYuZhuan/shiguang_warehouse  （MIT，作者 星河欲转）
    //   上游快照：2026-09-12；学校名与登录地址取自同目录 adapters.yaml（maintainer 星河欲转）。
    // 上游自称「强智适配」，脚本请求 /jsxsd/xskb/xskb_list.do —— 与同批的 hynu（衡阳师范学院）
    //   是同一平台的近克隆：同一个接口、同一个请求体、同一套 div.kbcontent + font[title=…] 结构。
    //
    // 取数方式：DOM 抓取（强智的课表页返回服务端渲染的 HTML，不是 JSON）。三级兜底：
    //   ① 当前页面就有课表表格 → 直接用（用户此刻看到的那张，也不多发请求）
    //   ② GET  /jsxsd/xskb/xskb_list.do   （教务默认的当前学期）
    //   ③ POST /jsxsd/xskb/xskb_list.do   （参数与上游一字不差：cj0701id=&zc=&demo=&xnxq01id=<学期>）
    //
    // 移植改动（与 hynu 的 extract.js 逐处对照见同目录 AUDIT.md）：
    ...
    //   ④ 报错信息带上当前页面主机名：这所学校的教务在 jw.hniu.cn，而登录会先经过校内统一身份
    //      认证 / 门户（authserver.hniu.cn、ehall.hniu.cn）。用户若停在门户页上点「提取课表」，
    //      请求会因跨源而失败，报错里说清「你现在在哪一页」比只说「连不上」有用。
    //
    // 只请求 jw.hniu.cn（同一台教务服务器）：不读账号密码、不写页面、不外发任何数据。
    var KB_URL = 'https://jw.hniu.cn/jsxsd/xskb/xskb_list.do';
    var LOGIN_HINT = '请先登录教务系统并打开「课表查询」页（https://jw.hniu.cn/jsxsd/），' +
        '确认页面上能看到课表再点「提取课表」';
    ...
    // 当前页面主机名，只进报错信息（不进载荷、不外发）
    function where() {
        var host = '';
        try {
            host = clean(location.host);
        } catch (e) {
            host = '';
        }
        return host ? '（当前页面：' + host + '）' : '';
    }
    ...
    function requestHtml(url, init) {
        return fetch(url, init).then(function (response) {
            if (response.status === 401 || response.status === 403) {
                throw new Error('教务系统拒绝访问（' + response.status + '）：' + LOGIN_HINT + where());
            }
            if (response.status < 200 || response.status >= 300) {
                throw new Error('教务系统返回 HTTP ' + response.status + '：' + LOGIN_HINT + where());
            }
            return response.text();
        }, function () {
            throw new Error('连不上教务系统：登录状态可能已失效。' + LOGIN_HINT + where());
        });
    }
    ...
    return requestHtml(KB_URL, { method: 'GET', credentials: 'include' }).then(function (html) {
        ...
        if (!got.term.code) {
            throw new Error('教务页面里没找到课表，也没找到学年学期下拉框：' + LOGIN_HINT + where());
        }
